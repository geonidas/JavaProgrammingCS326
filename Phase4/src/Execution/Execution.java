...
public class Execution {
    ...
    */
    public Execution(String className, String methodName, OperandStack os) {
        ...
        try {
    /*wtf*/        pw = new PrintWriter(new FileWriter("EVM-" + activation.threadCounter.getMaxCounter()+".log"), true);
        } catch (IOException e) {
...
/*Value cannot be converted to object_*/                    ReferenceValue rVal = new ReferenceValue(val);
                    //Get object_
/*cannot find getSuperClassObject*/                    Object_ iObj = new getSuperClassObject(rVal);
...
/*where was cName declared?*/                    if (cName == iObj.className()) // how do i check for inheritance?
                    {
/*where was cRinst declared? and use superClassObj variable in object_*/                      if(!(Object_.isAssignableForm(cRinst) && cRinst.isInterface()))
                ...
                case RuntimeConstants.opc_instanceof:// YOUR CODE HERE
                {
                  ...
                  ReferenceValue rVal = new ReferenceValue(val); //new?

                  //get the object from the ReferenceValue
                  Object_ iObj = new getSuperClassObject(rVal);
                  ...
                  if (cName == iObj.className()) // how do i check for inheritance?
                  {
                    if(Object_.isAssignableForm(cRinst) && cRinst.isInterface())
                      operandStack.push(1);
                    else
                      operandStack.push(0);
                  }
                ...
                case RuntimeConstants.opc_invokevirtual: // YOUR CODE HERE
                {
                    ...
                    //get the name of the classes
                    //get the name of the method
                    //get the class object that represents the type of class
                ...
                case RuntimeConstants.opc_lreturn:
                {
                    ...
                    //2) otherwise check if the function returns something which can by checking opcode, pop value and ass to stack of the activation of the caller
                ...
                case RuntimeConstants.opc_new: // YOUR CODE HERE
                {
                    ...
                    Class theClass = ClassList.getClass(cName);
                    Object_ cObj = new getSuperClassObject(theClass); //should i use "new"?
